...
namespace NokPortalAPI.Services.Old
{
    public class UserAppsService : IUserAppsService
    {
        ...
        /// <returns></returns>
        public async Task<(AppEnvironment, JwtResponse)> GetJWTForTargetAppAsync(int appId, EnumEnvironmentType env)
        {
            using var connection = connectionFactory.CreateConnection();
            connection.Open();
            using var tran = connection.BeginTransaction();
            try
            {
                AppEnvironment appEnv = await appEnvRepository.GetByIdAsync(connection, appId, env, tran);
        ...
        public async Task<(AppEnvironment, JwtResponse)> GetJWTTokenTargetAppWithData(int appId, EnumEnvironmentType env, IUser user, IEnumerable<AppWithRoles> userAppsList)
        {
            ...
            try
            {
                ...
                JwtResponse jwtToken = jwtService.GenerateTokenForTargetAppWithData(jwtSetting, appEnv.SecretKey, appEnv.JwtExpirationHour);
                tran.Commit();
