...
namespace NokPortal.Domians.Controllers
{
    ...
    public class AppController : NokController<ControllerBase>
    {
        ...
        public async Task<ActionResult<ApiResponse<object, string>>> CreateUserApp(int id)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(FormatInvalidFieldResponse(GetFieldErrors()));
            ...
            int userId = _managePayload.GetUserIdFromJwtDecode(HttpContext);
            UserApp userApp = await _usersAppService.GetUserAppAsync(id, userId);

            if (userApp.RoleId != UserRole.Root && userApp.RoleId != UserRole.Admin)
            {
            ...
            try
            {
                await _usersAppService.AddUserAppAsync(userApp); // (int)UserRole.User
                return Ok(FormatSuccessResponse(null));
            }
            catch (ArgumentNullException ex)
            {
