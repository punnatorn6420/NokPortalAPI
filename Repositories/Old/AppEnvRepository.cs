...
namespace NokPortalAPI.Repositories.Old
{
    ...
    public class AppEnvRepository : IAppEnvRepository
    {
        ...
        public async Task CreateAsync(IDbConnection conn, AppEnvironment appEnv, IDbTransaction? tran = null)
        {
        ...
        public async Task UpdateAsync(IDbConnection conn, AppEnvironment appEnv, IDbTransaction? tran = null)
        {
            const string checkSql = @"
                SELECT COUNT(*)
                FROM Apps_Env
        ...
        public async Task<AppEnvironment> GetByIdForAdminAsync(IDbConnection conn, int appID, int userID, EnumEnvironmentType env, IDbTransaction? tran = null)
        {
            ...
            AppEnvironment? appEnv = await conn.QuerySingleOrDefaultAsync<AppEnvironment>(sql, new { AppID = appID, Env = (int)env }, tran);
        ...
        /// <returns></returns>
        /// <exception cref="DataValidationException"></exception>
        public async Task<AppEnvironment> GetByIdAsync(IDbConnection conn, int appID, EnumEnvironmentType env, IDbTransaction? tran = null)
        {
            const string sql = "SELECT * FROM Apps_Env WHERE AppID = @AppID AND Environment = @Env";
            AppEnvironment? appEnv = await conn.QuerySingleOrDefaultAsync<AppEnvironment>(sql, new { AppID = appID, Env = (int)env }, tran);
        ...
        /// <param name="tran">Transaction to use.</param>
        /// <returns></returns>
        public async Task<IEnumerable<AppEnvironment>> GetAllAsync(IDbConnection conn, IDbTransaction? tran = null)
        {
            const string sql = "SELECT * FROM Apps_Env";
            return await conn.QueryAsync<AppEnvironment>(sql, transaction: tran);
        }
